...
from innout.models.cnn1d import Net, OneDCNN
...
class NIR(object):
    def __init__(self, args):
        ...
        config = dict()
        root = os.path.join(args.root, args.dataset_clear+".json")
        # 读取 JSON 文件并转换为字典
        ...
        iid_transmittance_spec, iid_label, wavelength, self.main_scaler = self.get_data(iid_data, args.label, args.dataset_clear)
        # 数据集划分
        iid_train, iid_test, iid_train_label, iid_test_label = train_test_split(
            iid_transmittance_spec, iid_label, test_size=0.2, random_state=args.seed
        )
        ood_val, ood_val_label, _, _ = self.get_data(ood_data, args.label, args.dataset_clear, self.main_scaler)
        ...
        self.ood_loader = DataLoader(ood_dataset, batch_size=args.batch_size, shuffle=True)
        self.test_loader = DataLoader(test_dataset, batch_size=args.batch_size, shuffle=True)

        self.mlp = OneDCNN(input_dim=iid_transmittance_spec.shape[1])

    def get_data(self, root, label,dataset, scaler_train=None):
        Transmittance_spec = []
        labels = []
    ...
    def fetch_train(self):
        ...
        train_z = torch.mean(train_x, dim=2)
        train_g = bs[1]
        # train_c = torch.unsqueeze(random_zero_one(train_y.shape[0]), 1)
        return (
            train_x.cuda(),
    ...
    def fetch_test(self):
        ...
        test_z = torch.mean(test_x, dim=2)
        test_g = bs[1]
        return (
            test_x.cuda(),
            # test_y.cuda(),
    ...
    def fetch_val(self):
        bs = next(iter(self.ood_loader))
        val_x = bs[0]
        val_y = bs[1]
        val_x = val_x.unsqueeze(1)
        val_z = torch.mean(val_x, dim=2)
        val_g = bs[1]
        return (
            val_x.cuda(),
            # val_y.cuda(),
...
def evaluate_split(model, batch_num, fetch_fn, meta_acc, main_scaler, split_name="val"):
    model.eval()
    meta_acc.clear()
    ...
    for _ in range(batch_num):
        x, y, z, g, c, inv_noise = fetch_fn()
        logits = model(x)

        # 用于精度评估
        meta_acc.process_batch(g, logits)

        # 反归一化处理预测和标签（用于MSE/MAE）
        y_pred_np = logits.detach().cpu().numpy().reshape(-1, 1)
        y_true_np = g.detach().cpu().numpy().reshape(-1, 1)

        y_pred_orig = main_scaler.inverse_transform(y_pred_np).squeeze()
        y_true_orig = main_scaler.inverse_transform(y_true_np).squeeze()

        all_preds.extend(y_pred_orig.tolist())
        all_labels.extend(y_true_orig.tolist())

    # === 回归指标 ===
    mse = mean_squared_error(all_labels, all_preds)
    mae = mean_absolute_error(all_labels, all_preds)
    mape = np.mean(np.abs((np.array(all_labels) - np.array(all_preds)) / np.array(all_labels)))

    # === 环境分类指标（如果有）===
    res = meta_acc.meta_acc
    acc = float(res.get(f"{split_name}_acc", 0.0))
    acc_worst = float(res.get(f"{split_name}_acc_worst", 0.0))

    return mse, mae, acc, acc_worst, mape
