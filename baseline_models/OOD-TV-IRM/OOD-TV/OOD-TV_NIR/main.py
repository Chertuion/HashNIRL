...
parser.add_argument('--dataset_clear', choices=['Chunjian_size', 'Chunjian_area', 'manggo_set', 'manggo_region', 'manggo_HarvestTime', 'manggo_maturity'], default='Chunjian_size', type=str, help='dataset name')
...
for restart in range(1):
    ...
    for step in tqdm(range(flags.steps)):
        ...
        if step % flags.print_every == 0:
            ...
            # ====== 评估 test 上表现 ======
            test_mse, test_mae, test_acc, test_worst, test_mape = evaluate_split(
            ...
            if test_mse < best_result_record.get("test_mse", float("inf")):
                ...
                best_result_record["test_mape"] = test_mape
                best_result_record["test_worst"] = test_worst
                best_result_record["test_epoch"] = step

                # ====== 在 test 最好时，记录 val 表现 ======
                ood_mse, ood_mae, ood_acc, val_worst, ood_mape = evaluate_split(
                ...
                best_result_record["ood_mape_when_test_best"] = ood_mape
                best_result_record["ood_worst_when_test_best"] = val_worst

                print(f"[Step {step}] 🟢 New best test MSE: {test_mse:.4f}, MAE: {test_mae:.4f}")
                print(f"→ Corresponding ood MSE: {ood_mse:.4f}, MAE: {ood_mae:.4f}, Mape: {ood_mape}")
