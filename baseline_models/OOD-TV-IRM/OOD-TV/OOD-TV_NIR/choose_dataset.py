from utils_z import CELEBAZ_FEATURE, HousePrice, LOGITZ, LOGIT2Z, LANDCOVER, ADULT,MNIST,NIR
from utils import (
    LOGIT_LYDP,
...
def init_nir(flags):
    dp = NIR(flags)
    mlp = dp.fetch_mlp().cuda()
    print(summary(mlp, input_size=(1, 200)))
    n_parameters = count_parameters(mlp)
    mlp2 = MLP2LayerParameter(flags, feature_dim=n_parameters, hidden_dim=128).cuda()
    print(summary(mlp2, input_size=(1, n_parameters)))
    test_batch_num = len(dp.test_loader)
    train_batch_num = len(dp.train_loader)
    val_batch_num = len(dp.ood_loader)
    test_batch_fetcher = dp.fetch_test
    val_batch_fetcher  = dp.fetch_val
    main_scaler = dp.main_scaler
    mean_nll = mean_nll_reg  # CrossEntropyLoss
    mean_accuracy = mean_accuracy_reg  # test acc
    eval_acc = eval_acc_reg  # train acc
    return (
        dp,
        mlp,
        mlp2,
